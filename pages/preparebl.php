...
<?php
	if(isset($_POST['delete'])){//delete
		...
		if(!empty($myid)){
			if ($ri==1){
				...
				$delete=mysql_query("delete from gl_caption WHERE ca_id='$myid' and ca_isbl=1");
			}
			else{
				$delete=mysql_query("delete from gllink WHERE `id` ='$myid' and isbl=1");
			}
			echo"<script>alert('Deleted Successfull!!');</script>";
			echo"<script>window.location.href='?pages=preparebl&catch=rollback';</script>";
		}
	}
	if(isset($_POST['update'])){//update
		...
		if(!empty($myid)){
			...
			if ($main=="Root"){
				$id=0;	
				//$main='';
				$display_info="SELECT indexs FROM gl_caption WHERE ca_master='$main' order by indexs desc limit 0,1";
			...
			//Get Info again
			$display_info="SELECT * FROM gl_caption WHERE ca_id='$myid' and   ca_isbl  =1 limit 0,1";
				$result_info=mysql_query($display_info) or die (mysql_error());
			...
			if ($ri==1){
				if(!empty($name)){
					...
					$insert_user=mysql_query("UPDATE `gl_caption` SET `ca_name` = '$name',
									...
									`ca_master` = '$main',
									`indexs` = '$indexs'
									 WHERE `ca_id` ='$myid' and ca_isbl=1;");
	...
	if(isset($_GET['action'])=='edit'){//view edit
				...
				$display_info="SELECT * FROM gl_caption WHERE  ca_id ='$myid' and  ca_isbl =1 and ca_level='$level' limit 0,1";
				$result_info=mysql_query($display_info) or die (mysql_error());
				$active_num=mysql_num_rows($result_info);
					while($row=mysql_fetch_array($result_info)){
					...
					if($dca_master==''){
						...
						$display_info="SELECT * FROM gllink WHERE  id ='$myid' and  isbl =1 limit 0,1";
						$result_info=mysql_query($display_info) or die (mysql_error());
						$active_num=mysql_num_rows($result_info);
						while($row=mysql_fetch_array($result_info)){
...
?>
<!-- Form -->
			<h3 class="tit">Prepare Balance Sheet Form :</h3>
            <fieldset>
                ...
                <tr>
                                ...
                                <select class="input-text" name="main">
                                    <option value="Root" selected="selected">--Root--</option>
                                     <?php 
                                        ...
                                        $str_recom="select ca_name,ca_id,indexs from gl_caption where ca_isbl=1";
                    ...
                    <td>Account :&nbsp;&nbsp;<input type="text" size="30" onclick="doselect()" class="input-text" readonly="readonly" name="acc" id="acc" onKeyUp="doSuggestionBox(this.value);" autocomplete="off" value="<?php
							 if(isset($_GET['action'])=='edit'){
							 	echo $dglcode;
							 }
							 else{
...
<!-- Table (TABLE) -->
				...
				<tr>
                    ...
                    <th>Type</th>
                    <th>Actions</th>
                    <th><a href="?pages=preparebl&clear" title="Add Prepare Balance Sheet"><img src="design/Add.png" border="0"/></a></th>
				</tr>
                </thead>
				<?php
					...
					while($row=mysql_fetch_array($result))
					{
						...
						$c_name=$row['ca_name'];
						$level=$row['ca_level'];
						$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount from gllink g , chartacc_list c , account a   where a.branch='$get_br' and isbl  =1 and ca_id='$id' and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl order by gl_code;";
						...
						<tr>
							...
							<td><a href='?pages=preparebl&id=$id&action=edit&catch=done&level=$level' title='Edit User' name='edit'>Edit</a></td>
							<td>&nbsp;</td>
						
							
						</tr>";
							$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount from (gllink g left join account a on g.gl_code=a.c_code) inner join chartacc_list c on g.gl_code=c.c_code   where  isbl  =1 and ca_id='$id' group by gl_code,c.c_name,id,isbl order by gl_code;";
							$result_acc=mysql_query($show_acc);
							while($row=mysql_fetch_array($result_acc))
							{
