...
<!-- end suggestion cif -->
<?php
		$query = "SELECT * FROM register WHERE bloan='0' ORDER BY max_id ASC";
		...
		//---check display----------------
	  	if(isset($_POST['display'])){
			...
			$check_dis=mysql_num_rows($result_reginfo);
						...
						while($row = mysql_fetch_array($result_pn))
								{
									...
									if($days_pn>=4){
										if ($remain==0){
											$due_pn=round(($sch_pn*$rate/100)*$days_pn*2/$Num_of_rep1,2);
										}
										else
										{
											$due_pn=round(($remain*$rate/100)*$days_pn*2/$Num_of_rep1,2);
										}			
									}
									else{
										$due_pn=0.00;
				...
				$sum_OP_P=mysql_query("SELECT ((SUM(interest-int_paid)-$po_pn1_First)*$Po_setting)/100 as PO_P FROM schedule WHERE  ld='$lid' AND rp='0'");
					...
					while($m<=$count_row)
					{
						...
						if ($sql_date1<$month){
							...
							while($row=mysql_fetch_array($sum_tt))
							{
								...
								$total_late_last = round($total_late_last+$int_late,2);
								$due_late=round($due_late+$due_pn_late,2);
								$due_pn=$due_pn_late;
				...
				$total_pn=formatMoney(round($due_late+$remain_pn,2),true);
				...
				*/
				...
				$myodtotal_tt=formatMoney(round($myodprn_tt+$T_P+$total_late_last+$po_pn1_First,2),true);
...
<h3 class="tit">Easy Pay Off Form :</h3>
        ...
        <input type="hidden" class="input-text" name="po_late_total" id="po_late_total" autocomplete="off" value="<?php echo $total_late_last; ?>"/>
         ...
         <tr>
			...
			<td>
            <input type="text" class="input-text" name="due_pn" id="due_pn" value="<?php echo $due_pn; ?>" readonly="readonly"/> 
            </td>
            <th valign="top">Addional Int :</th>
			<td>
            <input type="text" class="input-text" name="adInt" id="adInt" value="<?php echo $po_pn1_First; ?>" readonly="readonly"/>
            </td>
         ...
         <tr>
			...
			<td>
            <input type="text" class="input-text" name="po_pn" id="po_pn" value="<?php echo $po_pn1; ?>" readonly="readonly"/>
            </td>
