...
<!-- end suggestion cif -->
<?php
	$query = "SELECT cid FROM loan_process GROUP BY cid ASC";
        ...
        if($result) {
            while($row = mysql_fetch_array($result)) {
                echo("this.nameArray[" . $counter . "] = '" . trim($row['cid']) . " ';");
                $counter += 1;
            }
        }
        echo("</script>");
?>
<h3 class="tit">Search Loan Info :</h3>

		<!-- start id-form -->
        <form name="cust_request" method="post" enctype="multipart/form-data" action="pages/show_loanInfo_list.php" target="_new">
