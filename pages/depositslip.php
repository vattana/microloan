...
<?php
			...
			$tp=$_GET['t'];
						...
						if($tp=='rp'){
							$mytp='ប័ណ្ណដាក់ប្រាក់';
							$enTp='DEPOSIT VOUCHER';
							}
						else{
							$mytp='ប័ណ្ណបើកប្រាក់';	
			...
			$display_info="SELECT * FROM deposit WHERE ld='$lid' ORDER BY id ASC";
			$result_info=mysql_query($display_info) or die (mysql_error());
				while($row=mysql_fetch_array($result_info)){
						$regId = $row['reg_id'];
						$ld=$row['ld'];
						$saleDay=date('d',strtotime($row['date']));
						$saleMonth=date('m',strtotime($row['date']));
						$saleYear=date('Y',strtotime($row['date']));
						$myamt = $row['amount'];	
						$isfix = $row['isfix'];
						$period = $row['period'];
						$f_rate = $row['frate'] . ' %';
						...
						if ($isfix == 1){
							$DepositType='Fix Deposit';	
						}
						else{
							$DepositType='Current Deposit';	
						}
						if($cur=='USD'){
							$mycur='ដុល្លារ';
		...
		<table width='1000' height='auto' border='1' cellpadding='10' cellspacing='10'>
			<tr height='50'>
				<td align='left' width='290'>
					លេខគណនី / Account Number : $getStatus
				</td>
				<td align='center'>
					$lid
				</td>
				<td valign='middle' align='left' width='320'>
					កាលបរិច្ឆេទបញ្ញើ (Date):$saleDay/$saleMonth/$saleYear
				</td>
			</tr>
			<tr height='50'>
				<td align='left' width='290'>
				   ...
				   <table border='0' width='100%'>
						...
						<tr>
							...
							<br>
								...
								ហត្តលេខាអតិថិជន Customer's Signature
							</td>
					...
					<table border='0' width='100%' height='135'>
						<tr>
							<td colspan='2' align='center' valign='top' bgcolor='#999999' height='20'>
								<div style='font-family:Khmer OS Muol; font-size:10pt; text-decoration:none'>សម្រាប់សារីណា FOR SARYNA USE ONLY</div>
							</td>
						</tr>
				   		<tr>
							<td align='center' class='footer_report' valign='bottom'>
