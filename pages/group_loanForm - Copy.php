...
<!-- end suggestion cif -->
<?php
		$query = "SELECT cif FROM register where appr='1' and cancel ='0' and bcif='1' and bloan='0' and dis='0' and type_loan='group' 
		GROUP BY cif DESC";
		...
		$result_info=mysql_query($display_info) or die (mysql_error());
			while($row=mysql_fetch_array($result_info)){
				...
				//-----------------
					///check charactor
					if($loan_rows=='0'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-A';
					}
					else if($loan_rows=='1'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-B';
					}
					else if($loan_rows=='2'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-C';
					}
					else if($loan_rows=='3'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-D';
					}
					else if($loan_rows=='4'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-E';	
					}
					else if($loan_rows=='5'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-F';
					}
					else if($loan_rows=='6'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-G';
					}
					else if($loan_rows=='7'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-H';
					}
					else if($loan_rows=='8'){
						$ld=$code_l_type.$freq.'-'.$get_loan_month.$get_loan_year.'-'.$code_lcycle.$code_cur_type.$max.'-I';
					}
					else{
						echo"<script>confirm('your member is > 8 ,continoue?');</script>";
						}
			}///isset
	//--check ld
		if($ld==''){
						...
						if($method=='Declining' || $method=='Negotiation'){ //start decline
							...
							if(!empty($installment_date) && !empty($myreg_date)&& !empty($reg_id) && ($myclassi_purpose!='0')){
									...
									$insert_date="INSERT INTO schedule(id,record_date,cid,ld,no_install,repayment_date,principal,interest,total,balance,response_co,currency,br_no)
											...
											mysql_close();
											echo"<script>window.location.href='index.php?pages=noneCollateral_loanForm';</script>";
																}
...
<!-- start content -->
<div id="content">
<div id="page-heading"><h1>Easy Group Loan Form Group ID (<? echo $gid; ?>):</h1></div>
...
<tr>
	...
	<td>	
        ...
        <th>&nbsp;</th>
        <td><a href="index.php?pages=search_gid_list&gid=<? echo $gid; ?>&catch=enc_64">Search Group ID</a></td>
        </tr>
        <tr>
		<th valign="top">&nbsp;</th>
		<td>	
		...
		<tr>
           ...
           onclick="return confirm('Are you sure wanna save ? Please check :Setting and Classify Purpose before you click ok.');"/>
