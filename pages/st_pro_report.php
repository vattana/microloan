...
$d =date('Y-m-d');
	...
	echo "
		<table width='1090' height='auto' border='0' cellpadding='0' cellspacing='1'>
			...
			<tr>
				<td colspan='5' align='center'><u><h5>Income Statement</h5></u></td>
			</tr>
...
<table width="1090" height="auto" border="0" class="sortable resizable editable" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
	...
	<?php
			...
			//////show schedule
					...
					while($row=mysql_fetch_array($result))
					{
						...
						if ($br=='0'){
							...
							$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit,a.c_cur from gllink g , chartacc_list c , account a   where a.branch='$mybr' and a.a_date between '$from' and '$to' and isbl  =0 and ca_id='$id'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit,a.c_cur order by gl_code;";
						}
						
						/*$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit from gllink g , chartacc_list c , account a   where isbl  =0 and ca_id='$id' and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit order by gl_code;";*/
							...
							if ($amountc=='0' && $amountd=='0'){
								if ($br=='0'){
									$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit,a.c_cur from gllink g , chartacc_list c , account a   where a.a_date between '$from' and '$to' and isbl  =0 and ca_id='$id1'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit,a.a_date,a.c_cur order by gl_code;";
								}
								else{
									$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit,a.c_cur from gllink g , chartacc_list c , account a   where a.branch='$mybr' and a.a_date between '$from' and '$to' and isbl  =0 and ca_id='$id1'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit,a.c_cur order by gl_code;";
								}
								/*$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit from gllink g , chartacc_list c , account a   where isbl  =1 and ca_id='$id1'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit order by gl_code;";*/
								$result_acc=mysql_query($show_acc);
								while($row=mysql_fetch_array($result_acc))
								{
									$isdebit = $row['c_is_debit'];
				...
				echo"</tr>";
							if ($br=='0'){
								$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit,a.c_cur from gllink g , chartacc_list c , account a   where a.a_date between '$from' and '$to' and isbl  =0 and ca_id='$id'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit,a.c_cur,a.a_date order by gl_code;";
							}
							else{
								$show_acc="select gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit,a.c_cur from gllink g , chartacc_list c , account a   where a.branch='$mybr' and a.a_date between '$from' and '$to' and isbl  =0 and ca_id='$id'  and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit,a.c_cur order by gl_code;";
							}
		...
		//	$n+=1;	
			...
			/////////////////////////total credit
			//gl_code , c.c_name,id,sum(c_debit+c_credit) amount,c.c_is_debit from gllink g , chartacc_list c , account a   where isbl  =0 and ca_id='$id' and g.gl_code=a.c_code and g.gl_code=c.c_code group by gl_code,c.c_name,id,isbl,c.c_is_debit order by gl_code
