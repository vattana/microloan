...
<!-- end suggestion cif -->
<?php
		$query = "SELECT * FROM loan_process WHERE dis='0' ORDER BY max_id ASC";
...
<?php
		...
		//---check display----------------
	  	...
	  	if(isset($_POST['display'])){
			...
			$display_appinfo="SELECT * FROM customer_app WHERE cid='$cid' and register_date='$myregDate'";
			...
			while($row=mysql_fetch_array($result_reginfo)){
				...
				//check if already disburse
					if($check_dis=='0'){
						...
						echo"<script>window.location.href='index.php?pages=disForm&catch';</script>";
						}
				//////end check disburse
			}///isset displaay
	...
	//---------start disbursement----------------//
		if(isset($_POST['disburse'])){//start schedule 
			...
			$spit = array(",", "'");
 			$myloan = str_replace($spit, "",$_POST['loan_amt']);
			$mycur=$_POST['cur'];
			$myld = $_POST['lid'];
			
			$display_reginfo="SELECT glcode FROM loan_process WHERE id='$myld'";
		$result_reginfo=mysql_query($display_reginfo) or die (mysql_error());
			while($row=mysql_fetch_array($result_reginfo)){
					$mygl=$row['glcode'];
				}
			...
			if(!empty($myreg_id)){
				$update_dis="UPDATE loan_process SET dis_date ='$dis_date',dis='1',status='1' WHERE regis_id='$myreg_id'";
				...
				$update_disapp="UPDATE customer_app SET dis ='1' WHERE cid='$mycid' and register_date='$myregDate'";
					mysql_query($update_disapp) or die (mysql_error());
			}
			
			$display_reginfo="SELECT * FROM glp_acc WHERE glcode='$mygl' and glp_acc.desc='Principal'";
		$result_reginfo=mysql_query($display_reginfo) or die (mysql_error());
			$i=0;
			while($row=mysql_fetch_array($result_reginfo)){
					$code=$row['credit'];
					...
					INSERT INTO `account` (
							...
							VALUES (
						'$code', '$mycur', '$myloan', '0', null, '', '$dis_date','$myld','1','$get_br');
					") or die(mysql_error);
						$i++;
					}
					else{
					...
					INSERT INTO `account` (
							...
							VALUES (
						'$code1', '$mycur', '0', '$myloan', null, '', '$dis_date','$myld','1','$get_br');
					") or die(mysql_error);
					}
			}
			...
			/*echo"
					<script> 
				var printdis =window.open('','print','status=1,width=1100,height=700,scrollbars=yes,menubar=yes,addressbar=no,resizable=no');
						printdis.location.href='pages/transactionSlip.php?lid=$lid&cid=$cid&inNo=$max&t=d&lcred=$myloan&cur=$mycur';
					</script>";*/
			
			echo "<script>alert('Disbursement Successful');</script>";
			/*echo"<script>window.location.href='index.php?pages=disForm';</script>";*/
			
		}//end isset schedule
		
	//------------end disbursment---------------// 
	?>
<h3 class="tit">Easy Disbursement Form :</h3>
		<!-- start id-form -->
        ...
        <tr>
         ...
         <tr>
            ...
            <?php echo $l_currency; ?>
            <input type="hidden" name="cur" id="cur" value="<?php echo $l_currency; ?>"/>
            </td>
        ...
        onkeypress="return handleEnter(this, event);" value="<?php echo $first_repay; ?>" readonly="readonly"/>
         ...
         <tr>
			...
			<td>
            <input type="text" class="input-text" name="method" id="method" value="<?php echo $method; ?>" readonly="readonly"/>
            </td>
		</tr>
		...
		<td>	
		<input type="text" class="input-text" name="ddate" id="ddate" autocomplete="off" onblur="doDate(this,'em_Date');" 
		...
		<th valign="top">Classified Purpose :</th>
            <td><textarea rows="" cols="20" class="form-textarea" name="loan_purpose" id="loan_purpose"><?php echo trim($classify_pur); ?></textarea></td>
            <td>&nbsp;</td>
		</tr>
		<tr>
            <th>&nbsp;</th>
