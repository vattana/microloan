<?php
	...
	if(isset($_POST['submit'])){
			...
			$mycur=$_POST['cur'];
			if(!empty($myloan) && !empty($app_int) && !empty($app_period)){
				$app_sql=mysql_query("INSERT INTO `customer_app` (
							...
							VALUES (
							NULL , '$id', '$cid', '$de_reg_date', '$app_date', '$myloan', '$app_int', '$app_period', '$method_select', '$no_repay', '$user_app','$recom_name','$co', '','$get_br','$mycur'
							);");
							$update_register=mysql_query("UPDATE register SET appr='1' WHERE id='$id'");
							
							if($num_rows_rcycle>1){
								$update_bcif=mysql_query("UPDATE register SET bcif='1' WHERE id='$id'");
								}
							echo"<script>alert('$borrower is Approved Successful!');</script>" ;
...
<head>
<title>Approval Form - OLMS</title>
...
<div id="container">
	<table width="1200" border="0">
     ...
     <fieldset class="action">
    	<div style="color:#FFF; font-size:20">Approval Form for : <?php echo $borrower .' with CID '.$cid ;?></div>
    </fieldset>
<form method="post" class="niceform" onsubmit="return formCheck(this);">
    ...
    <fieldset>
     ...
     <fieldset class="action">
        ...
        onclick="return confirm('Are you sure want to approve <?php echo $borrower; ?> ?');"/>
