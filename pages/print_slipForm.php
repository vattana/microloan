<script type="text/javascript">
	function focusit() {			
		document.getElementById("inNo").focus();
		}
	...
	<script type="text/javascript">
	 ...
	 <script type="text/javascript">
            ...
            function doSelection(text) {
                var selection = text;
                document.getElementById('divSuggestions').style.visibility = 'hidden'; // hides the suggestion box
                document.getElementById("inNo").value = selection;
            }
...
<!-- end suggestion cif -->
<?php
		$query = "SELECT * FROM invoice ORDER BY invioce_no DESC";
			...
			if($result) {
				while($row = mysql_fetch_array($result)) {
					echo("this.nameArray[" . $counter . "] = '" . trim($row['invioce_no']) . " ';");
					$counter += 1;
				}
			}
			echo("</script>");
?>
<h3 class="tit">Print Transaction Slips :</h3>
		<!-- start id-form -->
        <form name="sch" method="post" enctype="multipart/form-data" action="pages/transactionSlip.php" target="_blank">
		...
		<td>	
		<input type="text" class="input-text" name="inNo" id="inNo" autocomplete="off" 
