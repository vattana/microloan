		...
		//---check display----------------
	  	if(isset($_POST['display'])){
			...
			$result_khrinfo=mysql_query($round_khinfo) or die (mysql_error());
						...
						while($row = mysql_fetch_array($result_pn))
								{
									...
									if($days_pn>=4){
										if ($remain==0){
												if ($l_currency=='KHR'){
												$due_pn=roundkhr(round(($sch_pn*$rate/100)*$days_pn*3/$Num_of_rep1,2),$round_khr);
												
											}
											else if($l_currency=='THB'){
												$due_pn=formatmoney(round(($sch_pn*$rate/100)*$days_pn*3/$Num_of_rep1,2),true);
											}
											else{
												$due_pn=formatmoney(round(($sch_pn*$rate/100)*$days_pn*3/$Num_of_rep1,2),true);
											}
										...
										{
											if ($l_currency=='KHR'){
												$due_pn=roundkhr(round(($remain*$rate/100)*$days_pn*3/$Num_of_rep1,2),$round_khr);
											}
											else if($l_currency=='THB'){
												$due_pn=formatmoney(round(($remain*$rate/100)*$days_pn*3/$Num_of_rep1,2),true);
											}
											else{
												$due_pn=formatmoney(round(($remain*$rate/100)*$days_pn*3/$Num_of_rep1,2),true);
											}
				...
				if ($l_currency=='KHR'){
												$overdue_pn=formatMoney(roundkhr(round($pn_tt['pn_tt'],2),$set),true);
				...
				//End
				//Early Time
					if ($due_date<$repay_date){ // Before OS + Total on schedule + remain interest (30%)
						
						$sum_int=mysql_query("SELECT interest-int_paid as int_tt, principal-prn_paid as prn_tt FROM schedule WHERE ld='$lid' AND rp='0' order by repayment_date limit 1");
						$d=0;
						$int_last=0;
						while($row=mysql_fetch_array($sum_int))
						{	
				...
				$pn_p=mysql_fetch_array($pn_paid);
					...
					if ($l_currency=='KHR'){
												...
												$remain_pn=roundkhr(round(trim(str_replace(",","",$overdue_pn))-trim(str_replace(",","",$paid_pn)),2),$round_khr);
				...
				if ($due_late==0)
				{
					...
					if ($l_currency=='KHR'){
												$total_pn=formatMoney(roundkhr(round(trim(str_replace(",","",$due_late))+trim(str_replace(",","",$remain_pn)),2),$round_khr),true);
												
											}
											else if($l_currency=='THB'){
												$total_pn=intval(trim(str_replace(",","",$due_late))+trim(str_replace(",","",$remain_pn)),2);
											}
											else{
				...
				$prn_tt=mysql_fetch_array($sum_prn);
					if ($l_currency=='KHR'){
												$myodprn_tt=formatMoney(roundkhr($prn_tt['prn_tt']+$int_first+$prn_first,$round_khr),true);
											}
											else if($l_currency=='THB'){
												$myodprn_tt= formatMoney(round($prn_tt['prn_tt']+$int_first+$prn_first,2),true);
											}
											else{
												$myodprn_tt= formatMoney(round($prn_tt['prn_tt']+$int_first+$prn_first,2),true);
											}
				...
				*/
				...
				//$toal_tt=mysql_fetch_array($total_int);
					...
					if ($l_currency=='KHR'){
												...
												$T_P = formatMoney(round(trim(str_replace(",","",$total_pn)) + trim(str_replace(",","",$po_pn1))),true);
											}
											else{
												$T_P = formatMoney(round(trim(str_replace(",","",$total_pn)) + trim(str_replace(",","",$po_pn1))),true);
	...
	//---------start disbursement----------------//
		...
		//	$total_pn_org=$_POST['repay_penalty'];
			...
			$result_info=mysql_query($display_info) or die (mysql_error());
				...
				//display approval info
				$Num_of_rep=0;
				$display_appinfo="SELECT * FROM customer_app WHERE cid='$reg_id'";
				$result_appinfo=mysql_query($display_appinfo) or die (mysql_error());
				while($row=mysql_fetch_array($result_appinfo)){
						$rate = $row['approval_rate'];
						$Num_of_rep=$row['number_of_repay'];
				...
				else
				{
					if ($addi_int!=0){
						...
						if ($mycur=='KHR'){
							...
							$po_pn1_First =roundkhr(round($po_pn1_First,2),$set);
						}
						else if($mycur=='THB'){
							$po_pn1_First =$interest+$addi_int;
							$po_pn1_First =round($po_pn1_First,2);
				...
				if($mycur=='KHR'){
					$prin_1 = roundkhr(round($Sum_OP_tt_First['principal'],2),$set);	
				} 
				else if($mycur=='THB'){
					$prin_1 = round($Sum_OP_tt_First['principal'],2);	
				}
				else{
					$prin_1 = round($Sum_OP_tt_First['principal'],2);	
				}
				...
				$result_mininstall= $Sum_OP_tt_First['no_install'];
					...
					$update_sch="UPDATE schedule SET prn_paid ='$prin_1',int_paid='$interest',rp='9',paid_date ='$month' 
				...
				while($row=mysql_fetch_array($count_install)){
					...
					if ($ta==1){
						if ($due_date<$repay_date){
							...
							$sum_OP_P=mysql_query("SELECT ((interest-int_paid-$po_pn1_First)) as PO_P FROM schedule WHERE  ld='$lid' AND rp='0' and no_install='$no_ins'");
						}
					}
					else{
						$sum_OP_P=mysql_query("SELECT ((interest-int_paid-$po_pn1_First)*$Po_setting)/100 as PO_P FROM schedule WHERE  ld='$lid' AND rp='0' and no_install='$no_ins'");	
					...
					while($row=mysql_fetch_array($sum_OP_P)){
						$po_pn1= roundkhr(round($row['PO_P'],2),$set);
					}
				...
				$prn = str_replace(",","",$prn);
				$po_int = str_replace(",","",$po_int);
				$total_pn_all = str_replace(",","",$total_pn_all);
				$invoice_exc="INSERT INTO invoice(id,reg_id,lc,paid_date,sch_date,invioce_no,prn_paid,int_paid,pn_paid,int_dis,payoff_pnpaid,total,cashier,pay_type,pay_status,reciev_at,max_id,res_co,cur) VALUES (null,'$myreg_id', '$lid', '$repay_date', '$sch_date', '$max','$prn','$po_int','$total_pn_all','0','0', '$total', '$user','$dis_type', 'po', '$get_br', '$max', '$myco', '$mycur');";
					...
					$loan_status=mysql_query("update schedule set rp='9' where ld='" . $lid . "'");
					echo"
					<script> 
...
<h3 class="tit">Easy Pay Off Form :</h3>
		...
		<tr>
			...
			<td>
            <input type="text" class="input-text" name="due_amt" id="due_amt" value="<?php echo $due_amt; ?>" readonly/>
            </td>
        ...
        <tr>
			...
			<td>
            <input type="text" class="input-text" name="due_prn" id="due_prn" value="<?php echo $due_prn; ?>" readonly/>
            </td>
            <th valign="top">Total Interest:</th>
			<td>
            <input type="text" class="input-text" name="repay_int" id="repay_int" autocomplete="off" value="<?php echo formatmoney(str_replace(",","",$myodint_tt),true); ?>"/>
            </td>
		</tr>
        <tr>
			<th valign="top">Due Interest :</th>
			<td>
            <input type="text" class="input-text" name="due_int" id="due_int" value="<?php echo $due_int; ?>" readonly/>
            </td>
        ...
        <input type="hidden" class="input-text" name="po_late_total" id="po_late_total" autocomplete="off" value="<?php echo $total_late_last; ?>"/>
         ...
         <tr>
			...
			<td>
            <input type="text" class="input-text" name="adInt" id="adInt" value="<?php
			 echo "0";
			 //echo formatmoney(roundkhr($po_pn1_First,$set),true); 
			 ?>" readonly/>
            </td>
         ...
         <tr>
            ...
            <input type="text" class="input-text" name="po_pn" id="po_pn" value="<?php 
					if ($l_currency=='KHR'){
						echo formatmoney(roundkhr(intval(str_replace(",","",$po_pn1)),$round_khr),true); 
					}
					else if($l_currency=='THB'){
						
						echo formatmoney($po_pn1,true); 
					}
					else{
			...
			<td>
            <input type="text" class="input-text" name="to_pen" id="to_pen" readonly value="<?php echo $T_P; ?>" />
            </td>
         ...
         <tr>	
			...
			<td>
            <input type="text" class="input-text" name="cur" id="cur" value="<?php echo $l_currency; ?>" readonly/>
            </td>
        </tr>
         <tr>	
        	<td colspan="2">&nbsp;
        ...
        <tr>
            ...
            onkeypress="return isNumberKey(event);" autocomplete="off" readonly/></td>
            <th valign="top"></th>
			<td>
