...
<?php
		...
		//---check display----------------
	  	...
	  	if(isset($_POST['display'])){
			 ...
			 if($rp!='0'){
					...
					echo"<script>window.location.href='index.php?pages=delete_disForm';</script>";
					exit();
				 }
			//display loan info
			...
			///////////ens get infor from approval	
				...
				$display_reginfo="SELECT * FROM register WHERE id='$reg_id' AND appr='1' AND cancel='0' AND bcif='1' AND bloan='1' AND dis='1' ORDER BY id DESC";
	...
	//---------start disbursement----------------//
		if(isset($_POST['delete'])){//start schedule 
			...
			if(!empty($myreg_id)){
				...
				$update_register="UPDATE register SET dis='0' WHERE id='$myreg_id'";
					mysql_query($update_register) or die (mysql_error());
					
				$update_app="UPDATE customer_app SET dis='0' WHERE cid='$mycid'";
					mysql_query($update_app) or die (mysql_error());
					
				$update_loanpro="UPDATE loan_process SET dis_date='0000-00-00',dis='0',status='0' WHERE regis_id='$myreg_id'";
					mysql_query($update_loanpro) or die (mysql_error());
					
				$update_sch="UPDATE schedule SET dis='0' WHERE ld='$mylid'";
					mysql_query($update_sch) or die (mysql_error());
			}
				$delete_acc="delete from account WHERE c_des='$mylid'";
					mysql_query($delete_acc) or die (mysql_error());
			
			
			echo"<script>alert('The lD: $mylid has been deleted from disbursement transaction!');</script>";
	...
	//------------end disbursment---------------// 
	?>
<h3 class="tit">Delete Disbursement Transaction Form :</h3>
		<!-- start id-form -->
		...
		<th valign="top">Disbursment Date :</th>
		<td>	
		<input type="text" class="input-text" name="disdate" id="disdate" autocomplete="off" onkeypress="return handleEnter(this, event);" 
        value="<?php echo $disdate; ?>" readonly="readonly"/>
		</td>
		</tr> 
		<tr>
		...
		<tr>
            ...
            onclick="return confirm('Are you sure wanna delete disbursment entry of <?php echo $kh_borrower; ?> ?');"/>
