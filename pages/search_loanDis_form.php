...
<?php
	$query = "SELECT ld FROM loan_process where dis='1' GROUP BY ld ASC";
        ...
        echo("</script>");
?>
<h3 class="tit">Search Loan Disbursment :</h3>
		<!-- start id-form -->
        <form name="cust_request" method="post" enctype="multipart/form-data" action="pages/show_loanDis_list.php" target="_new">
                        ...
                        <select class="input-text" name="recommender" id="recommender">
                            <option value="0">--Recommender--</option>
                             <?php 
								...
								$str_recom="Select recom_name from loan_process Group by recom_name asc";
								$sql_recom=mysql_query($str_recom);
								while ($row=mysql_fetch_array($sql_recom))
								{
									$recomend_name=$row['recom_name'];
			...
			<td>
            	<select class="input-text" name="co" id="co" onkeypress="return handleEnter(this, event);">
                           ...
                           <?php 
								...
								$str_co="Select co from loan_process Group by co asc";
