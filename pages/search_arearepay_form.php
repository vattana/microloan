...
<?php
	$query = "SELECT ld FROM loan_process where dis ='1' GROUP BY ld ASC";
        ...
        echo("</script>");
?>
<h3 class="tit">Search Area Repayment :</h3>
		<!-- start id-form -->
        <form name="cust_request" method="post" enctype="multipart/form-data" action="pages/show_arearepay_list.php" target="_new">
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" width="100%" class="nostyle">
		<tr>
			<th valign="top">Date :</th>
			<td><input type="text" class="input-text" name="from" id="from" autocomplete="off" onblur="doDate(this,'em_Date');" 
			...
			<td>
            	<select class="input-text" name="co" id="co" onkeypress="return handleEnter(this, event);">
                           ...
                           <?php 
								...
								$str_co="Select response_co from schedule Group by response_co asc";
								$sql_co=mysql_query($str_co);
								while ($row=mysql_fetch_array($sql_co))
								{
									$res_co=$row['response_co'];
