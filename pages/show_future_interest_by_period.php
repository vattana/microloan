...
<title>Future Interest By Period:</title>
...
<?php 					
			...
			/*echo"<script>alert('$type_of_staff,$performance,$br_no,$from,$to');</script>";*/
					echo "<h2><u>Future Interest By Period</u></h2>";
					echo"
			...
			<table width="1000px" cellpadding="0" cellspacing="0" height="auto" class="form_border" style="margin:0px; border-collapse:collapse" border="1" >
							...
							<tr align="center" style=" background-color:#06F;color:#FFF;" height="28">
                                ...
                                <td width="180"><b>Future Interest</b></td>
                                <td width="180"><b>Date Saving</b></td>
                            ...
                            <tbody>
                            	<?php
									$dp_info="Select distinct ld,cid,borrower,sex,cust_dob,amount,((amount*(frate/365))/100)*d.period FI,d.date date_saving,d.cur,d.period from deposit d,register r where  d.cid=r.cif and r.type_cust ='new' and d.isfix=1 and d.date between '$from' and '$to' and d.cur='$getCur'";
						$result_dp=mysql_query($dp_info) or die (mysql_error());
							...
							while($row = mysql_fetch_array($result_dp)){
								...
								if ($getCur=='KHR'){
									...
									$FI = formatmoney(roundkhr($row['FI'],$set));
								}
								else if($getCur=='THB'){
									$amount = formatmoney(intval($row['amount'],2));
									$FI = formatmoney(intval($row['FI'],2));	
								}
								else{
									$amount = formatmoney(round($row['amount'],2));
									$FI = formatmoney(round($row['FI'],2));
								}
								$date_saving = $row['date_saving'];
								$cur = $row['cur'];
								$FI = $row['FI'];
								$period = $row['period'];
								echo "<tr>
