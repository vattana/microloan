...
<?php
	  	...
	  	if(isset($_POST['display'])){
			...
			$result_appinfo=mysql_query($display_appinfo) or die (mysql_error());
				while($row=mysql_fetch_array($result_appinfo)){
						...
						$noInst = $row['approval_period'];
					}
			// check last installment 
			$counIns=mysql_query("SELECT * FROM schedule WHERE ld='$lid' and rp='1';");
			$numInst=mysql_num_rows($counIns);
			if($numInst == ($noInst-1)){
				echo"<script>alert('អតិថិជននេះនៅសល់តែមួយ Installment ប៉ុណ្ណោះមិនទាន់បង់រួច ,លោកអ្នកអាចបង់ផ្តាច់រឺក៍បង់មួយចំនួនសូមអរគុណ!');</script>";
				}
			//-----start show currency
			...
			$result_khrinfo=mysql_query($round_khinfo) or die (mysql_error());
						...
						while($row = mysql_fetch_array($result_pn))
								{
									...
									if($days_pn>=$setLpn){
										...
										{
											if ($l_currency=='KHR'){
												$due_pn=roundkhr(($sch_pn*$rate/100)*$days_pn*2/$frq,$round_khr);
											}
											else if($l_currency=='THB'){
												$due_pn=intval(($sch_pn*$rate/100)*$days_pn*2/$frq,2);	
											}
											else{
										...
										{
											if ($l_currency=='KHR'){
												$due_pn=roundkhr(($get_remain*$rate/100)*$days_pn*2/$frq,$round_khr);
											}
											else if($l_currency=='THB'){
												$due_pn=intval(($get_remain*$rate/100)*$days_pn*2/$frq,2);
											}
											else{
				...
				if ($l_currency=='KHR'){
												...
												$paid_pn=roundkhr(trim(str_replace(",","",$overdue_pn))-trim(str_replace(",","",$paid_pn)),$round_khr);
											}
											else if($l_currency=='THB'){
	...
	//---------start disbursement----------------//
		if(isset($_POST['repayment'])){//start schedule 
			...
			$total_pn=str_replace(",","",$_POST['due_pn']);
		...
		//	$total_pn_org=$_POST['repay_penalty'];
			...
			//Check amount >= balance
				...
				while ($amount>0){
						...
						$countInstall = " SELECT approval_period - (SELECT count( no_install ) FROM schedule WHERE schedule.ld = L.ld AND STATUS =1 AND rp =1) as no_install FROM customer_app C INNER JOIN loan_process L ON C.id = L.regis_id  where L.ld='$lid'";
						...
						{
							print("<script>alert('amount bigger than balance, so u should payoff this load.');</script>");
						...
						$amount_org=0.0;
						$amount_org = $int_from+$prin_from;
						$amount_o = $amount;
						$amount_bal= $amount_o-round($amount_org,2);	
						...
						if ($amount_bal>=round(0,2)){
							$j+=1;
							$int_end= $int_from+$int_paid-$int_dis;
							$prin_end=$prin_from+$prin_pain;
							$rp=1;
							if ($j==1){ //have penalty
									$update_sch="UPDATE schedule SET prn_paid='$prin_end',int_paid='$int_end',rp='$rp',paid_date ='$repay_date' 
							...
							$prn_pain_total = $prn_pain_total + $prin_end-$prin_pain;
							$int_pain_total = $int_pain_total + $int_end-$int_paid;
				...
				$total = $prn_pain_total+ $int_pain_total +$normaltotalpenalty;
				//insert invoice
			   // $amount = $_POST['g_amt'];
...
<h3 class="tit">Easy General Repayment Form :</h3>
        ...
        <input type="hidden" class="input-text" name="repay_penalty_remain" id="repay_penalty_remain" autocomplete="off" value="<?php echo "00"; ?>"/>
         ...
         <tr>
			...
			<td>
            <input type="text" class="input-text" name="due_pn" id="due_pn" value="<?php echo formatmoney($due_pn,true); ?>" readonly/> 
            </td>
			 <th valign="top">Remain :</th>
			<td>
            <input type="text" class="input-text" name="remain" id="remain" readonly value="00"/>
            </td>
		</tr>
         <tr>
			<th valign="top">Interest Discount :</th>
			<td>
            <input type="text" class="input-text" name="int_dis" id="int_dis" value="<?php echo $int_dis; ?>" readonly/>
            </td>
		...
		<tr>
            ...
            onclick="return confirm('Are you sure want to wave penalty for <?php echo $kh_borrower; ?> ?');"/>
            </td>
            <td>
           </td>
	 </tr>
