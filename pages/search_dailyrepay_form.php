...
?>
<!-- start content-outer -->
<h3 class="tit">Search Daily Repayment :</h3>
		<!-- start id-form -->
        <form name="cust_request" method="post" enctype="multipart/form-data" action="pages/show_dailyrepay_list.php" target="_new">
        ...
        <tr>
			...
			<td>	
                        <select class="input-text" name="cashier" id="cashier">
                            <option value="0">--Receiver--</option>
                             <?php 
							
								$str_recom="Select cashier from invoice Group by cashier asc";
			...
			<td>
            	<select class="input-text" name="co" id="co" onkeypress="return handleEnter(this, event);">
                           ...
                           <?php 
								...
								$str_co="Select res_co from invoice Group by res_co asc";
								$sql_co=mysql_query($str_co);
								while ($row=mysql_fetch_array($sql_co))
								{
									$res_co=$row['res_co'];
									echo '<option value="'.$res_co.'">' .$res_co. ' </option>';
								}
							?>
                        </select>
            </td>
		...
		<td>	
		<input type="text" class="input-text" name="lid" id="lid" autocomplete="off" onKeyPress="return handleEnter(this, event);" 
